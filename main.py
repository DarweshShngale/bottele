...
للتحويل من الإيزيدية إلى العربية، أرسل النص الإيزيدي مباشرة."""
...
البوت سيتعرف تلقائياً على نوع النص ويقوم بالتحويل المناسب."""
...
def convert_text(message):
    ...
    if any(char in yazidi_to_ar for char in text):
        ...
        response = f"النص بالعربية:\n{result}"
    else:
        result = convert_to_yazidi(text)
        response = f"النص بالإيزيدية:\n{result}"
